-- Analytics Tables for AcademiQR
-- This creates tables to track link clicks, page views, and visitor data
...
CREATE TABLE IF NOT EXISTS link_clicks (
    ...
    CONSTRAINT link_clicks_link_id_fkey FOREIGN KEY (link_id) REFERENCES link_items(id) ON DELETE CASCADE,
    CONSTRAINT link_clicks_list_id_fkey FOREIGN KEY (list_id) REFERENCES link_lists(id) ON DELETE CASCADE,
    CONSTRAINT link_clicks_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES auth.users(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS page_views (
    ...
    CONSTRAINT page_views_list_id_fkey FOREIGN KEY (list_id) REFERENCES link_lists(id) ON DELETE CASCADE,
    CONSTRAINT page_views_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES auth.users(id) ON DELETE CASCADE
);

-- 3. Create indexes for better query performance
CREATE INDEX IF NOT EXISTS idx_link_clicks_link_id ON link_clicks(link_id);
CREATE INDEX IF NOT EXISTS idx_link_clicks_list_id ON link_clicks(list_id);
CREATE INDEX IF NOT EXISTS idx_link_clicks_owner_id ON link_clicks(owner_id);
CREATE INDEX IF NOT EXISTS idx_link_clicks_clicked_at ON link_clicks(clicked_at DESC);

CREATE INDEX IF NOT EXISTS idx_page_views_list_id ON page_views(list_id);
