import{s as u}from"./supabase-whCCoo43.js";import{r as R,b as D,c as M,g as U,a as z}from"./link-utils-D1VnX3pm.js";const k=document.getElementById("public-root");function c(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function H(e){try{return new Date(e+"T00:00:00").toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}catch{return e}}function N(e,t){const i=e.presentation_data||{},o=i.title||"AcademiQR Collection",n=(t==null?void 0:t.display_name)||"",a=i.conference||"",s=[];n&&s.push(n),a&&s.push(a);const l=s.length>0?`${o} — ${s.join(" • ")}`:o,m=(t==null?void 0:t.profile_photo)||"https://natzpfyxpuycsuuzbqrd.supabase.co/storage/v1/object/public/assets/AcademiQR_logo_Dark.png";document.title=`${o} | AcademiQR`;const b={"og:title":o,"og:description":l,"og:image":m,"og:type":"website","og:url":window.location.href,"twitter:card":"summary","twitter:title":o,"twitter:description":l,"twitter:image":m};for(const[g,$]of Object.entries(b)){const y=g.startsWith("twitter:")?"name":"property";let p=document.querySelector(`meta[${y}="${g}"]`);p||(p=document.createElement("meta"),p.setAttribute(y,g),document.head.appendChild(p)),p.setAttribute("content",$)}let f=document.querySelector('meta[name="description"]');f||(f=document.createElement("meta"),f.setAttribute("name","description"),document.head.appendChild(f)),f.setAttribute("content",l)}function G(e=50,t=50,i=100){return`translate(${(e-50)*.6}%, ${(t-50)*.6}%) scale(${i/100})`}function Q(e){const t={backgroundType:"solid",backgroundColor:"#ffffff",gradientText:"",backgroundImage:"",backgroundImageX:50,backgroundImageY:50,backgroundImageScale:100,profileTextColor:"#1A2F5B",presentationTextColor:"#1A2F5B",buttonTextColor:"#000000",buttonBackgroundColor:"#1A2F5B",buttonStyle:"soft",buttonBorderRadius:"8px",borderEnabled:!1,borderType:"solid",borderStyle:"fill",borderColor:"#1A2F5B",borderGradient:""};if(!e)return{...t};if(typeof e=="string")try{e=JSON.parse(e)}catch{return{...t}}const i=!!(e.borderEnabled||e.gradientBorderEnabled);return{backgroundType:e.backgroundType||"solid",backgroundColor:e.backgroundColor||"#ffffff",gradientText:e.gradientText||"",backgroundImage:e.backgroundImage||"",backgroundImageX:e.backgroundImageX??e.imagePositionX??50,backgroundImageY:e.backgroundImageY??e.imagePositionY??50,backgroundImageScale:e.backgroundImageScale??e.imageScale??100,profileTextColor:[e.textColor,e.presentationTextColor,e.profileTextColor,e.presentationColor,e.profileColor].find(o=>typeof o=="string"&&o.length>0)||"#1A2F5B",presentationTextColor:[e.textColor,e.presentationTextColor,e.profileTextColor,e.presentationColor,e.profileColor].find(o=>typeof o=="string"&&o.length>0)||"#1A2F5B",buttonTextColor:e.buttonTextColor||"#000000",buttonBackgroundColor:e.buttonBackgroundColor||e.buttonBgColor||"#1A2F5B",buttonStyle:e.buttonStyle||"soft",buttonBorderRadius:e.buttonBorderRadius||e.borderRadius||"8px",borderEnabled:i,borderType:e.borderType||"solid",borderStyle:e.borderStyle==="fill"||e.borderStyle==="thin"?e.borderStyle:"fill",borderColor:e.borderColor||"#1A2F5B",borderGradient:e.borderGradient||e.borderGradientText||""}}function v(e,t){k.innerHTML=`
    <div class="error-state">
      <i class="fas fa-exclamation-circle"></i>
      <h2>${c(e)}</h2>
      <p>${c(t)}</p>
    </div>
  `}async function X(e){const{data:t,error:i}=await u.from("link_lists").select("id, owner_id, slug, visibility, passkey_hash, theme, presentation_data, qr_code_data").eq("id",e).single();if(i)throw i;return t}async function Y(e){const{data:t,error:i}=await u.from("link_items").select("id, url, title, image_url, image_position, image_scale, order_index, is_active, source_link_id, use_library_defaults, custom_overrides").eq("list_id",e).order("order_index",{ascending:!0});if(i)throw i;const o=(t||[]).filter(n=>n.is_active!==!1);return await R(o),o}async function j(e){const{data:t,error:i}=await u.from("profiles").select("display_name, profile_photo, profile_photo_position, social_email, social_instagram, social_facebook, social_twitter, social_linkedin, social_youtube, social_tiktok, social_snapchat").eq("id",e).single();if(i)throw i;return t||{}}const A=crypto.randomUUID();async function w(e){try{const t=navigator.userAgent,i=/Mobile|Android|iPhone|iPad|iPod/i.test(t),n=/iPad|Android(?!.*Mobile)/i.test(t)?"tablet":i?"mobile":"desktop";await u.from("page_views").insert({list_id:e.id,owner_id:e.owner_id,user_agent:t.substring(0,500),device_type:n,referrer:document.referrer||null,session_id:A})}catch{}}function O(e,t){u.from("link_clicks").insert({link_id:t,list_id:e.id,owner_id:e.owner_id,user_agent:navigator.userAgent.substring(0,500),session_id:A}).then(()=>{}).catch(()=>{})}function J(e,t){u.from("link_clicks").insert({list_id:e.id,owner_id:e.owner_id,user_agent:navigator.userAgent.substring(0,500),social_platform:t,session_id:A}).then(()=>{}).catch(()=>{})}function V(e){const{buttonStyle:t,buttonBackgroundColor:i,buttonTextColor:o}=e;let n=`color: ${o}; border-radius: 8px; font-size: 1rem;`;return t==="solid"?n+=`background: ${i}; border-color: ${i};`:t==="outline"?n+=`background: transparent; border: 2px solid ${o}; color: ${o};`:n+=`color: ${o};`,n}function L(e,t,i){const o=Q(e.theme),n=e.presentation_data||{},a=i||{};document.title=n.title?`${n.title} — AcademiQR`:"AcademiQR";let s="";o.backgroundType==="gradient"&&o.gradientText?s=`background: ${o.gradientText};`:o.backgroundType==="image"&&o.backgroundImage?s=`background: url('${o.backgroundImage}') ${o.backgroundImageX}% ${o.backgroundImageY}% / ${o.backgroundImageScale}% no-repeat;`:s=`background: ${o.backgroundColor};`;let l="background: #1e293b;";o.borderEnabled&&(o.borderType==="gradient"&&o.borderGradient?l=`background: ${o.borderGradient};`:o.borderStyle==="thin"?l=`background: #1e293b; box-shadow: inset 0 0 0 8px ${o.borderColor};`:l=`background: ${o.borderColor};`);const m=a.profile_photo||"";let b={scale:100,x:50,y:50};if(a.profile_photo_position)try{b=typeof a.profile_photo_position=="string"?JSON.parse(a.profile_photo_position):a.profile_photo_position}catch{}const f=(b.scale||100)/100,g=((b.x||50)-50)*-1,$=((b.y||50)-50)*-1,y=[{key:"social_email",icon:"fa-envelope",prefix:"mailto:",fab:!1},{key:"social_instagram",icon:"fa-instagram",prefix:"",fab:!0},{key:"social_facebook",icon:"fa-facebook",prefix:"",fab:!0},{key:"social_twitter",icon:"fa-x-twitter",prefix:"",fab:!0},{key:"social_linkedin",icon:"fa-linkedin",prefix:"",fab:!0},{key:"social_youtube",icon:"fa-youtube",prefix:"",fab:!0},{key:"social_tiktok",icon:"fa-tiktok",prefix:"",fab:!0},{key:"social_snapchat",icon:"fa-snapchat",prefix:"",fab:!0}].filter(r=>{var d;return(d=a[r.key])==null?void 0:d.trim()}),p=n.title||"",E=n.displayTitle!==!1,B=n.displayConference!==!1,x=n.conference||"",T=n.location||"",C=n.date?H(n.date):"",q=E&&p||B&&x||T||C,I=o.profileTextColor,P=o.presentationTextColor;k.innerHTML=`
      ...
      <div class="public-frame" style="${l}">
        <div class="public-screen" style="${s}">
          ...
          <div class="public-header">
            <div class="public-profile">
              ${m?`
                <div class="public-avatar">
                  <img src="${c(m)}" alt="Profile"
                       style="transform: translate(${g}%, ${$}%) scale(${f}); transform-origin: center center;"
            ...
            ${q?`
              <div class="public-presentation" style="color: ${P};">
                ${E&&p?`<div class="public-info-field"><span class="public-info-value">${c(p)}</span></div>`:""}
          ...
          <div class="public-links">
            ...
            `:t.map(r=>{const d=D(r),h=M(r),S=G(d.x,d.y,h),_=U(r),F=z(r)||"Untitled";return`
