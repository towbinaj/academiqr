...
async function checkUsernameOnboarding() {
  try {
    ...
    document.getElementById('onboarding-save-btn')?.addEventListener('click', async () => {
      ...
      if (!val || val.length < 3) {
        alert('Username must be at least 3 characters (letters, numbers, hyphens only)')
        return
      }
...
function renderCollections() {
  ...
  if (collections.length === 0) {
    grid.innerHTML = `
      <div class="empty-state">
        ...
        <p>Create your first collection to start building link pages.</p>
        <button class="btn-primary" id="empty-create-btn">+ Create Collection</button>
      </div>
    `
  ...
  if (filtered.length === 0) {
    grid.innerHTML = `
      <div class="empty-state">
        ...
        <p>No collections match "${escapeHtml(searchQuery)}"</p>
      </div>
    `
    return
  }
  ...
  // Init drag-and-drop (only when not searching — reordering a filtered list is meaningless)
