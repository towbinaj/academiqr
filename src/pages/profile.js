...
async function saveProfile() {
  ...
  // If username is already set, don't allow changes (field is disabled, but enforce server-side too)
  ...
  for (const s of socials) {
    ...
    if (s === 'email') {
      socialData[`social_${s}`] = val // email doesn't need URL normalization
    } else {
  ...
  if (error) {
    ...
    const isUsernameTaken = error.message?.includes('username') || error.code === '23505'
    const msg = isUsernameTaken ? 'Username is already taken' : 'Save failed'
    btn.innerHTML = `<i class="fas fa-exclamation-triangle"></i> ${msg}`
    if (isUsernameTaken) {
      const status = document.getElementById('username-status')
      if (status) { status.textContent = 'Username is taken'; status.className = 'username-status taken' }
    }
    setTimeout(() => {
...
async function deleteAccount() {
  ...
  try {
    ...
    console.log('[Profile] Account deleted:', data?.deletionLog)
