...
import { isValidEmail, validatePassword, showMessage, setPersistentLogin, getPersistentLogin, clearPersistentLogin } from '../shared/utils.js'
...
async function init() {
  ...
  if (logoImg) {
    logoImg.addEventListener('load', () => { logoImg.style.display = 'block'; logoImg.nextElementSibling.style.display = 'none' })
    logoImg.addEventListener('error', () => { logoImg.style.display = 'none'; logoImg.nextElementSibling.style.display = 'block' })
  }
...
async function handleEmailLogin(event) {
  ...
  if (!isSignUpMode) {
    ...
    if (!serverLimit.allowed || serverLimit.isLocked) {
      ...
      showMessage(messageEl, `Too many failed attempts. Try again in ${minutes} minute${minutes !== 1 ? 's' : ''}.`, 'error')
      return
    }
  }
...
async function handleSignIn(email, password) {
  ...
  showMessage(messageEl, 'Successfully signed in!', 'success')
...
async function handleSignUp(email, password) {
  ...
  if (!validation.isValid) {
    showMessage(messageEl, 'Password does not meet requirements: ' + validation.errors.join(', '), 'error')
    return
  }
  ...
  if (!serverLimit.allowed || serverLimit.isLocked) {
    const minutes = Math.ceil((serverLimit.remainingTime || 0) / 60)
    showMessage(messageEl, `Too many sign-up attempts. Try again in ${minutes} minute${minutes !== 1 ? 's' : ''}.`, 'error')
    return
  }
  ...
  if (error) {
    ...
    if (error.message?.toLowerCase().includes('password')) {
      showMessage(messageEl, 'Password does not meet security requirements.', 'error')
    } else {
      showMessage(messageEl, 'Unable to create account. Please try again.', 'error')
    }
    return
  }
  ...
  showMessage(messageEl, 'Account created! Check your email for confirmation if required.', 'success')
...
async function handleGoogleLogin() {
  ...
  try {
    ...
    if (error) {
      showMessage(messageEl, 'Google login failed. Please try again.', 'error')
    }
...
async function handleForgotPassword(e) {
  ...
  if (!email) {
    showMessage(messageEl, 'Please enter your email address first, then click "Forgot your password?"', 'error')
    $('email')?.focus()
    return
  }
  ...
  try {
    ...
    showMessage(messageEl, 'Password reset email sent! Check your inbox for a link to reset your password.', 'success')
  } catch (err) {
    showMessage(messageEl, 'Failed to send reset email. Please try again.', 'error')
  } finally {
...
async function handlePasswordResetCallback() {
  ...
  document.getElementById('reset-form')?.addEventListener('submit', async (e) => {
    ...
    if (!validation.isValid) {
      showMessage(msg, 'Password does not meet requirements: ' + validation.errors.join(', '), 'error')
      return
    }
    ...
    try {
      ...
      if (error) throw error
      showMessage(msg, 'Password updated successfully! Redirecting...', 'success')
      setTimeout(() => navigate('dashboard'), 1500)
    } catch (err) {
      showMessage(msg, 'Failed to update password: ' + (err.message || 'Please try again.'), 'error')
      btn.disabled = false
