...
async function init() {
  ...
  linkSaver = createAutoSaver(async () => {
    ...
    try {
      ...
      // Only include tags if they exist (column may not exist before migration)
      if (link.tags && link.tags.length > 0) {
        updates.tags = link.tags
      }
...
function renderDetailsTab(container) {
      ...
      <div class="section-content" id="tags-section">
        <div class="form-group">
          <div class="editor-tags-container">
            ${renderTagChips(pd.tags || [], { removable: true })}
          </div>
        ...
        <div class="link-editor">
          ...
          <div class="form-group">
            ...
            ${currentImageUrl ? `
              ...
              <div class="form-group">
                ...
                <input type="range" id="link-img-scale" min="50" max="300" value="${imgScale}" class="range-input">
              </div>
              ` : ''}
            ` : ''}
          </div>
          ...
          <div class="form-group">
            ...
            <div id="link-tags-display" style="display:flex; flex-wrap:wrap; gap:4px; margin-bottom:6px;">
              ${renderTagChips(link.tags || [], { removable: true })}
            </div>
  ...
  // ── Bind Tags events ──
  const tagsContainer = container.querySelector('.editor-tags-container')
  const tagInputContainer = document.getElementById('editor-tag-input-container')
  if (tagInputContainer) {
    const currentTags = (collection.presentation_data || {}).tags || []
    createTagInput(tagInputContainer, allUserTags, currentTags, async (tag) => {
      const pd = collection.presentation_data || {}
      pd.tags = normalizeTags([...(pd.tags || []), tag])
      collection.presentation_data = pd
      if (!allUserTags.includes(tag)) { allUserTags.push(tag); allUserTags.sort(); invalidateTagCache() }
      presentationSaver.trigger()
      // Re-render tags chips
      if (tagsContainer) tagsContainer.innerHTML = renderTagChips(pd.tags, { removable: true })
      bindTagRemoveButtons(tagsContainer)
    })
  }
  bindTagRemoveButtons(tagsContainer)
  ...
  if (selectedLinkId) {
    ...
    // Link tags
    const linkTagsDisplay = document.getElementById('link-tags-display')
    const linkTagInputContainer = document.getElementById('link-tag-input-container')
    if (linkTagInputContainer) {
      const link = links.find(l => l.id === linkId)
      if (link) {
        const currentLinkTags = link.tags || []
        createTagInput(linkTagInputContainer, allUserTags, currentLinkTags, async (tag) => {
          link.tags = normalizeTags([...(link.tags || []), tag])
          if (linkTagsDisplay) {
            linkTagsDisplay.innerHTML = renderTagChips(link.tags, { removable: true })
            bindLinkTagRemoveButtons(linkTagsDisplay, link)
          }
          if (!allUserTags.includes(tag)) { allUserTags.push(tag); allUserTags.sort(); invalidateTagCache() }
          linkSaver.trigger()
        })
...
async function handleLinkImageUpload(e) {
  ...
  try {
    ...
    const publicUrl = await compressAndUpload(file, 'links', user.id, { maxWidth: 1200, maxHeight: 1200, quality: 0.75 })
...
function renderAppearanceTab(container) {
        ...
        <div class="section-content" id="background-section">
          ...
          <div id="bg-image-group" style="display:${bgType === 'image' ? 'block' : 'none'}">
            <div class="form-group">
              ...
              ${nt.backgroundImage && nt.backgroundType === 'image' ? `
  ...
  document.getElementById('bg-image-remove')?.addEventListener('click', () => {
    collection._pendingBgImage = null
    if (collection.theme) collection.theme.backgroundImage = ''
    // Switch to solid background type
    const solidRadio = document.querySelector('input[name="bg-type"][value="solid"]')
...
async function handleBgImageUpload(e) {
  ...
  try {
    ...
    showToast('Background image upload failed: ' + error.message, 'error')
  } finally {
    if (uploadBtn) {
      uploadBtn.disabled = false
      uploadBtn.innerHTML = originalHtml
...
function getThemeFromForm() {
  ...
  return {
    ...
    backgroundImage: bgType === 'image' ? (collection._pendingBgImage || (collection.theme || {}).backgroundImage || '') : '',
...
// ── Link Tag Remove Buttons ──
function bindLinkTagRemoveButtons(container, link) {
  ...
  container.querySelectorAll('.tag-remove').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      container.innerHTML = renderTagChips(link.tags, { removable: true })
      bindLinkTagRemoveButtons(container, link)
      linkSaver.trigger()
    })
  })
}

// ── Collection Tag Remove Buttons ──
function bindTagRemoveButtons(container) {
  ...
  container.querySelectorAll('.tag-remove').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      collection.presentation_data = pd
      presentationSaver.trigger()
      container.innerHTML = renderTagChips(pd.tags, { removable: true })
      bindTagRemoveButtons(container)
    })
  })
}
...
function setupMobileUI() {
  ...
  tabBar.innerHTML = `
    <button class="mobile-tab-btn ${activeTab === 'details' ? 'active' : ''}" data-tab="details">
      <i class="fas fa-file-alt"></i><span>Details</span>
    </button>
    <button class="mobile-tab-btn ${activeTab === 'appearance' ? 'active' : ''}" data-tab="appearance">
  ...
  fab.innerHTML = '<i class="fas fa-eye"></i>'
  fab.title = 'Preview'
  ...
  fab.addEventListener('click', () => {
    ...
    overlay.querySelector('.preview-overlay-close').addEventListener('click', () => overlay.remove())
