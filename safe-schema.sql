-- Safe Database Schema for AcademiQR
-- This handles existing tables and policies gracefully
-- Run this in your Supabase SQL Editor

-- 1. Create collections table (if not exists)
...
-- 2. Create links table (if not exists)
...
-- 10. Create or replace auto-profile function
...
-- 11. Create or replace user signup trigger
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
    AFTER INSERT ON auth.users
    FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- 12. Create performance indexes (if not exists)
...
-- 13. Add missing columns to existing tables (if they don't exist)
DO $$ 
BEGIN
    -- Add columns to collections if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'collections' AND column_name = 'description') THEN
        ALTER TABLE collections ADD COLUMN description TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'collections' AND column_name = 'visibility') THEN
        ALTER TABLE collections ADD COLUMN visibility TEXT DEFAULT 'private' CHECK (visibility IN ('public', 'unlisted', 'private'));
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'collections' AND column_name = 'passkey') THEN
        ALTER TABLE collections ADD COLUMN passkey TEXT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'links' AND column_name = 'image_url') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'links' AND column_name = 'updated_at') THEN
        ALTER TABLE links ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;
